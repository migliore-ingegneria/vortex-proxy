...
impl LinuxXdpLimiter {
    ...
    pub fn new(interface: &str, bpf_code: &[u8]) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
...
impl XdpRateLimiter for LinuxXdpLimiter {
    fn block_ip(&self, ip: IpAddr) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if let IpAddr::V4(ipv4) = ip {
            ...
            let mut matched_ips: HashMap<_, u32, u32> = HashMap::try_from(bpf.map_mut("BLOCKED_IPS").unwrap())?;
